...
# Les features sont les variables que nous allons utiliser pour predire le diagnostic
X = df[feature_cols]

# La cible est le diagnostic (maladie) que nous voulons predire
y = df['diagnostic']
...
# random_state permet de faire la séparation de mainère aléatoire mais reproductible.
# En fixant une valeur (ex: 42), on s'assure que la même séparation sera obtenue à chaque exécution du code.
...
# random forest : un ensemble de plusieurs arbres de division (decision trees) qui votent pour la classe finale
# Creer le modele
...
comparison = pd.DataFrame({
    ...
    # ligne 0 à 9 (10 patients) du test set
})
print(comparison)
